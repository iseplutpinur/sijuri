...
CREATE ALGORITHM=UNDEFINED VIEW `vpembimbing`  AS  select `skripsi`.`pembimbing1` AS `pembimbing`,`skripsi`.`disetujui` AS `disetujui` from `skripsi` union all select `skripsi`.`pembimbing2` AS `pembimbing`,`skripsi`.`disetujui` AS `disetujui` from `skripsi` ;
